...
import React, {
  useState, useEffect, useMemo, useCallback, useDeferredValue, useRef, memo,
} from 'react';
...
const TableRow = memo(({ item, index, isActive, onClick }: {
  item: Interaction;
  index: number;
  isActive: boolean;
  onClick: (item: Interaction) => void;
}) => {
  ...
  return (
    <tr
      ...
      <td className="imod-td" style={{ paddingLeft: 20 }}>
        <span className="imod-row-no">{String(index + 1).padStart(2, '0')}</span>
      </td>
...
const InteractionModeration: React.FC = () => {
  ...
  const openDrawer = useCallback(async (item: Interaction) => {
    if (activeRowRef.current === item.id && drawerOpen) {
