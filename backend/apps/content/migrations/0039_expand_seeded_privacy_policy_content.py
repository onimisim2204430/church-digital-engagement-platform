...
def upgrade_seeded_privacy_policy(apps, schema_editor):
    PrivacyPolicy = apps.get_model('content', 'PrivacyPolicy')

    # Only update records that still contain the original system-seeded content.
    for policy in PrivacyPolicy.objects.filter(title='Privacy Policy'):
        if (policy.content or '').strip() == OLD_DEFAULT_CONTENT.strip() and (
            (policy.updated_by or '').strip() in {'', 'system-seed'}
        ):
            policy.content = NEW_DEFAULT_CONTENT
            policy.updated_by = 'system-seed'
            policy.save(update_fields=['content', 'updated_by'])
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('content', '0038_seed_privacy_policy_default'),
    ]

    operations = [
        migrations.RunPython(upgrade_seeded_privacy_policy, noop_reverse),
    ]
